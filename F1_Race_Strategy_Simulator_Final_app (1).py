...
def generate_rain_windows(
    ...
    rain_laps = max(1, min(rain_laps, total_laps))

    # Rain timing is presented as broad, deterministic
    # race phases rather than an apparently exact/random
    # lap prediction. For a ~60 lap race this becomes:
    # Early 1-20, Mid 21-40, Late 41-end.
    phase_ranges = [
        ("Early rain", 1, min(20, total_laps)),
    ]

    if total_laps > 20:
        phase_ranges.append(
            ("Mid-race rain", 21, min(41, total_laps))
        )

    if total_laps > 40:
        phase_ranges.append(
            ("Late rain", 42, total_laps)
        )

    windows = []

    for name, range_start, range_end in phase_ranges:

        # Pick the middle of each broad phase as the
        # deterministic simulation point. The UI still
        # reports the broad approximate phase range.
        representative_start = int(round(
        ...
        representative_start = max(
            1,
            min(representative_start, max_start)
        )

        representative_end = min(
            representative_start + rain_laps - 1,
            total_laps
        )
        ...
        # ====================================================
            ...
            if best["rain_start"] is None:
                ...
                rain_text = (
                    f"{best['scenario']} "
                    f"(approx. Lap "
                    f"{best['rain_range_start']}"
                    f" – "
                    f"{best['rain_range_end']})"
                )
